...
proc specialDefine(conf: ConfigRef, key: string; pass: TCmdLinePass) =
  ...
  elif cmpIgnoreStyle(key, "nimNoRuntime") == 0:
    if pass in {passCmd1, passPP}:
      conf.globalOptions.incl optNoMain
      conf.globalOptions.incl optCompileOnly
      conf.options.excl optAssert
      conf.options.excl optStackTrace
      conf.options.excl optLineTrace
      conf.options.excl optLineDir
      conf.symbols["nimBareMetal"] = "true"
